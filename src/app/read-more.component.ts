...
@Component({    
    selector: 'read-more',
    template: `<div #mailDiv [innerHTML]="currentText | safeHtml" [style.font-size.px]="font" [className]="isCollapsed ? 'showhighlight' : ''" style="font-size: 14px; white-space: pre-wrap; text-align: justify;"><p></p></div><div class="show-more" [class.hidden]="hideToggle" (click)="toggleView()">
    <button class="readon_new" [innerHTML]="isCollapsed ? 'Read more' : 'Read less'"></button>
</div>`
