...
public files: Array<PlayerFile>  = [
  {
    url: 'http://quickread.developmentserver.co.in/storage/uploads/mp3/with_intro_15753716181289.mp3',
    name: 'My audio 1',
    artist: 'Me',
    album: 'My album'
  },
...
msaapPlaylist: Track[] = [];
  ...
  getSingleSummary(){
    ...
    this.api.getSingleSummary(this.filter).subscribe(j => {
      this.summary=j;
      this.summary.availableon=JSON.parse(JSON.stringify(this.summary.availableon));
      for (var key in this.summary.availableon) {

        if (this.summary.availableon[key] === null) {
          delete this.summary.availableon[key];
        }
      }
